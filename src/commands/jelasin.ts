...
export default async function jelasin(
  ...
  try {
    if (filepath) {
      ...
      let improvedPrompt = `
instruction: jelasin${prompt}
kode:
${kode}
`
      const AIResult = await generate(INSTRUCTION, improvedPrompt, config, HISTORY)
      const { deskripsi, cara_pakai } = parseText(AIResult)
